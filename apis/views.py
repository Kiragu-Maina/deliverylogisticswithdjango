    ...
    if request.user.is_authenticated:
        ...
        if request.method == "POST":
            ...
            if user is not None:
                ...
                response.set_cookie(
                    "username", user.username, max_age=60 * 60 * 24 * 365
                )  # expires in a year
                return response
            else:
                return JsonResponse(
                    {"error": "Please, try again"}, safe=False, status=401
                )
...
def home_page(request):
    ...
    if not username:
        ...
        def routeplan():
            ...
            return kenchiccnew.objects.filter(
                Q(Posting_Description__icontains="KCX 469Y")
                | Q(Posting_Description__icontains="KCX 850W")
                | Q(Posting_Description__icontains="KDK 643F")
                | Q(Posting_Description__icontains="KDH 659T")
                | Q(Posting_Description__icontains="KDK 014F")
                | Q(Posting_Description__icontains="KDD 090R")
                | Q(Posting_Description__icontains="KDD 205J")
                | Q(Posting_Description__icontains="KDD 206J")
                | Q(Posting_Description__icontains="KDE 017W")
                | Q(Posting_Description__icontains="KDG 805F")
                | Q(Posting_Description__icontains="KCF 445R")
                | Q(Posting_Description__icontains="KCH 314V")
                | Q(Posting_Description__icontains="KCH 316V")
                | Q(Posting_Description__icontains="KCL 442A")
                | Q(Posting_Description__icontains="KCU 236D")
                | Q(Posting_Description__icontains="KCU 237D")
                | Q(Posting_Description__icontains="KCU 410E")
                | Q(Posting_Description__icontains="KCX 833H")
                | Q(Posting_Description__icontains="KCZ 313B")
                | Q(Posting_Description__icontains="KCZ 314B")
                | Q(Posting_Description__icontains="KDE 958P")
                | Q(Posting_Description__icontains="KCX 806H")
                | Q(Posting_Description__icontains="KDE 848J")
                | Q(Posting_Description__icontains="KDH 424A")
                | Q(Posting_Description__icontains="KDG 240Y")
                | Q(Posting_Description__icontains="KDH 339E")
                | Q(Posting_Description__icontains="KDH 338E")
                | Q(Posting_Description__icontains="KCZ 792Y")
                | Q(Posting_Description__icontains="KDD 671A")
                | Q(Posting_Description__icontains="KDH 516N")
                | Q(Posting_Description__icontains="KDH 085N")
                | Q(Posting_Description__icontains="KDH 142V")
                | Q(Posting_Description__icontains="KDJ 574E")
                | Q(Posting_Description__icontains="KBJ 956K")
                | Q(Posting_Description__icontains="KCU 235D")
                | Q(Posting_Description__icontains="KCX 640F")
                | Q(Posting_Description__icontains="KDA 061G")
                | Q(Posting_Description__icontains="KDA 880B")
                | Q(Posting_Description__icontains="KDB 428S")
                | Q(Posting_Description__icontains="KDB 729Y")
                | Q(Posting_Description__icontains="KDD 110J")
            ).values_list("id")

        def routeandname(userid):
            return list(
                kenchiccnew.objects.exclude(Customer_Name__isnull=True)
        ...
        def userid(user):
            return kenchiccnew.objects.filter(
                Posting_Description__icontains=user
            ).values_list("id")

        def data(userid, userid2):
            data = kenchiccnew.objects.filter(id__range=(userid, userid2)).values_list(
        ...
        def pending_invoices(user):
            return list(
                PodStatus.objects.filter(
                    user__icontains=user, podstatus="POD Not Collected"
                ).values_list("shop_name", "OrderDate")
            )
        ...
        def collectedinvoices(user):
            return list(
                PodStatus.objects.filter(
                    user__icontains=user,
                    podstatus="POD Collected",
                    OrderDate__date=today,
                ).values_list("shop_name")
            )
        ...
        pending_forms = []
        for form in list(
            PendingForm.objects.filter(user__icontains=user).values_list("shop_name")
        ):
        ...
        pendinginvoices = [
            x for x in pendinginvoices if x[0] not in [y[0] for y in collectedinvoices]
        ]
...
def scan_invoice(request):
    ...
    image = cv2.imdecode(
        np.frombuffer(invoice_picture.read(), np.uint8), cv2.IMREAD_UNCHANGED
    )
    ...
    A4_height = 11.69
    src = np.array(
        [[0, 0], [0, A4_height], [A4_width, A4_height], [A4_width, 0]], dtype="float32"
    )
    dst = np.array(
        [
            [0, 0],
